...
def shifted_window(start,end,duration,context,offset):
 ws=max(0.0,start-context+offset);we=min(duration,end+context+offset)
...
def main():
 ...
 labels=json.loads(a.labels.read_text())['labels'];labels=[x for x in labels if not a.exchange_id or x['exchange_id'] in a.exchange_id];offsets=a.offset or [0.0];source=a.output_dir/'source-16khz.wav'
 ...
 for label in labels:
  for offset in offsets:
   start,end=float(label['start']),float(label['end']);ws,we=shifted_window(start,end,len(full)/sr,a.context,offset);window=torch.from_numpy(full[round(ws*sr):round(we*sr)]).float()
   wave8=torchaudio.functional.resample(window,16000,8000).unsqueeze(0);separated=separator.separate_batch(wave8)[0].detach().cpu();exchange=[]
