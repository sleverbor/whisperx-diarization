...
def recovery_windows(gap,duration,size=25.0,overlap=12.0,context=.5):
    if not all(math.isfinite(x) for x in (size,overlap)) or size<=0 or not 0<=overlap<size:raise ValueError('Invalid window size/overlap')
    if not math.isfinite(context) or context<0:raise ValueError('Context must be finite and nonnegative')
...
def main():
    ...
    parser.add_argument('--context-seconds',type=float,default=.5,help='Audio context on each side; words outside the gap are never added')
    ...
    if not math.isfinite(args.minimum_gap) or args.minimum_gap<=0:parser.error('Minimum gap must be positive')
    ...
    for gap_index,gap in enumerate(gaps):
        observations=[]
        for window_index,(left,right) in enumerate(recovery_windows(gap,duration,args.window_seconds,args.overlap_seconds,args.context_seconds)):
            ...
            for segment in segments:
                eligible=bool(segment.avg_logprob>=-1.0 and segment.no_speech_prob<=.6 and segment.compression_ratio<=2.4)
