...
By default, gaps must be at least two seconds long. Eight- and twelve-second windows are configurable, but lost the height question in the local noisy-clip test. Each gap is decoded in windows of at most 25 seconds, with 12 seconds of overlap for longer gaps and half a second of surrounding context. Voice activity filtering is disabled only for these small windows. There is no topic prompt and no video-specific phrase or identity rule.
