...
class AgentReviewTest(unittest.TestCase):
    def setUp(self):
        ...
        media=self.root/'video.mp4'; media.write_bytes(b'video')
        self.assertEqual(init_session(self.session,source,"video",media),2)
        self.assertEqual(load(self.session/'session.json')["media"], str(media.resolve()))
