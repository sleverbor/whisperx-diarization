"""Compare original, lightly denoised and fully denoised speech."""
...
def main():
 ...
 p.add_argument('--original',type=Path,required=True);p.add_argument('--light',type=Path,required=True);p.add_argument('--full',type=Path,required=True);p.add_argument('--output-dir',type=Path,required=True);p.add_argument('--source-offset',type=float,default=0);p.add_argument('--language',default='en');args=p.parse_args()
 args.output_dir.mkdir(parents=True,exist_ok=True)
 audio={}
 for name,path in [('original',args.original),('light',args.light),('full',args.full)]:
 ...
 summary={'model':'large-v2','device':device,'language':args.language,'vad_filter':False,'beam_size':5,'condition_on_previous_text':False,'word_timestamps':True,'source_offset_seconds':args.source_offset,'samples_16000':lengths,'runs':{},'versions':{name:version(name) for name in ['faster-whisper','torch','soundfile']}}
