...
| Joint audio evidence | Original, speech-filtered, and MossFormer2 family comparison | Found a stable ten-word target candidate inside one disputed sentence, rejected ownership propagation across a known mixed boundary, and separated target identity from unstable wording on a short reply. | Bind wording evidence to the audio variant's speaker evidence; correlated transforms count as one family and never validate wording automatically. |
| Independent acoustic rescoring | wav2vec2 CTC candidate ranking | Supported the baseline prefix on a longer target-separated passage but failed the short human-labeled reply and produced unusable greedy text on short crops. | Potential review evidence for longer clean separated speech; require abstention on acoustically collapsed short phrases. |
| Target-speaker activity | nomo-pVAD release 1.1 at 160 ms | Detected the 0.72-second target reply, rejected the following officer reply, and localized the target suffixes in both mixed sentences; also produced false target-active runs inside longer non-target speech. | Promising boundary evidence when combined with diarization and voice evidence; unsafe as a standalone audio mask. |
